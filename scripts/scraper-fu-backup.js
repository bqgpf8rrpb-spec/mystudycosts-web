...
async function scrapeFU() {
  ...
  try {
    ...
    // 5. Extraction - Anchor Strategy (Primary)
    console.log('🔗 Extracting partners using Anchor Strategy...');
    const partners = await page.evaluate(() => {
      ...
      console.log(`🎓 Found ${universityLinks.length} potential university links`);

      universityLinks.forEach((linkData, index) => {
        try {
          const link = linkData.element;

          // Extract university name from link text
          let name = linkData.text;

          // Try to find additional context from parent elements
          let country = 'N/A';
          let city = 'N/A';

          // Look at parent container for additional info
          const parent = link.parentElement;
          if (parent) {
            const parentText = parent.textContent || '';
            // Extract country/city from surrounding text
            const lines = parentText.split('\n').map(l => l.trim()).filter(l => l.length > 0);

            // Country is often in the expanded section context
            // City might be mentioned near the university name
            for (const line of lines) {
              if (line !== name && line.length > 2 && line.length < 50) {
                // This could be city or country info
                if (!city || city === 'N/A') {
                  city = line;
          ...
          results.push({
            ...
            country: country,
            city: city,
            website: linkData.href,
            home_university: 'FU Berlin'
          });

        } catch (error) {
          console.log(`⚠️ Error processing university link ${index}: ${error.message}`);
        }
      });

      console.log(`✅ Extracted ${results.length} partners from anchor strategy`);
      return results;
    });

    // Fallback: Class-based extraction
    if (partners.length === 0) {
      console.log('⚠️ No anchor results found, trying class-based extraction...');
      const classPartners = await page.evaluate(() => {
        const results = [];
        ...
        // Log class names of first 5 elements for debugging
        if (foundElements.length > 0) {
          console.log('🔍 Class names of first 5 elements:');
          for (let i = 0; i < Math.min(5, foundElements.length); i++) {
            const classes = foundElements[i].className || 'no-class';
            const text = foundElements[i].textContent?.trim().substring(0, 50) || 'no-text';
        ...
        foundElements.forEach((element, index) => {
          try {
            ...
            console.log(`⚠️ Error processing class element ${index}: ${error.message}`);
          }
        });

        return results.filter(p => p.name !== 'N/A' && p.name.length > 2);
      });

      if (classPartners.length > 0) {
        console.log(`📊 Found ${classPartners.length} partners via class-based extraction`);
        partners.push(...classPartners);
      }
    }

        // Strategy 2: Look for any text content that looks like university names
        if (results.length === 0) {
          const allTextNodes = [];
          ...
          console.log(`📝 Found ${allTextNodes.length} text nodes to analyze`);
          ...
          allTextNodes.forEach((text, index) => {
            if (text.toLowerCase().includes('university') ||
              ...
              if (parts.length > 0) {
                results.push({
                  name: parts[0],
                  country: parts.length > 1 ? parts[1] : 'N/A',
                  city: 'N/A',
                  website: '',
                  home_university: 'FU Berlin'
                });
        ...
        // Strategy 3: Look for any elements with substantial text content
        if (results.length === 0) {
          const substantialElements = document.querySelectorAll('div, span, p, li');
          substantialElements.forEach((element, index) => {
            try {
              ...
              if (text && text.length > 20 && text.length < 150 &&
                  ...
                  !text.toLowerCase().includes('impressum') &&
                  !element.closest('nav') &&
